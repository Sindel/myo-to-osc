...
parser.add_argument('-m', '--adapter', dest='adapter', default="/dev/ttyACM0", help='Bluetooth adapter to connect to.')
...
osc_client = udp_client.SimpleUDPClient(args.ipAddress, int(args.port))  # OSC Client for sending messages.
...
def proc_imu(quat, acc, gyro):
    osc_client.send_message("/ori", [1,2,3,4])
